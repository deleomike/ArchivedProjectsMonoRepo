//
//  main.cpp
//  AI - BotClean Partially Visible
//
//  Created by Michael DeLeo on 6/15/18.
...
void next_move(int posr, int posc, vector <string> board) {
    ...
    if (goals.size() == 0){
        int mean =( board.size() / 2) + 1;
        if (posr < mean && posc < board.size() -1){ //if bot is in top left
            cout << "RIGHT";
        }
        else if (posr > mean && posc > 1){  // if bot is in bottom right
            cout << "LEFT";
        }
        else if (posr < board.size() -1 && posc > mean){ // if bot is in top right
            cout << "DOWN";
        }
        else {
            cout << "UP";
