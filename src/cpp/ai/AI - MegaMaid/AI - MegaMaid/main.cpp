...
vector<string> finalPath(int h,int w, vector <string> grid);
...
void next_move(int posr, int posc, int dimh, int dimw, vector <string> board) {
    int linearPosition = posr * dimw + posc+1;
    ...
    vector<string> path = finalPath(dimh,dimw,nboard);
...
vector<string> finalPath(int h, int w, vector <string> grid){
    ...
    counter = 1;
    for (int i = 0; i < h; i++){    //goes up y axis
        for (int j = 0; j < w; j++){    //goes up x axis
            if (grid[i][j] == 'b'){
                Start = counter;
    ...
    int lastNode = Start;
    vector<string> s;
        if (lastNode == (finalPath[0] - 1)){
            s.push_back( "RIGHT");
        }
        else if (lastNode == (finalPath[0] +1)){
            s.push_back("LEFT");
        }
        else if (lastNode == (finalPath[0] - w)){
            s.push_back("DOWN");
        }
        else if (lastNode == (finalPath[0] + w)){
            s.push_back("UP");
...
/*
 ...
 first two are the coordinates of the bot
 second two are the size of the board
