...
def upload():
    ...
    try:
        concepts = clarifai_data.get_concepts()
        print("concepts:")
        print(concepts)

    except:
        print("Error: Concepts did not work")
        print(url)
    ...
    objects = ['beauty ', 'woman', 'hair', 'face', 'brown']
    emotions = ['LIKELY', 'VERY_UNLIKELY', 'VERY_UNLIKELY', 'VERY_UNLIKELY']
    emotion_data = image_request.black_magic(emotions)
    print("Emotion Data Converted")
    ...
    first_line, second_line, third_line = super_generator.main_shit(phrases, weather_data)
    return render_template('result.html', first_line=first_line,second_line=second_line, third_line=third_line)
